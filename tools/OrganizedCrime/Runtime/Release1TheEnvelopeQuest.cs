...
/// <summary>
/// The Envelope quest rendered through S1API. Structurally identical to
/// <see cref="Release1ShortNoticeQuest"/>: exactly one entry, created at construction, leaving the
/// whole frozen cash amount at the drop. Located through <c>QuestManager.GetQuestByName</c>, which
/// matches the quest's (get-only, abstract) <see cref="Title"/>; <see cref="DisplayTitle"/> doubles as
/// both the player-facing title and the S1API lookup key for this quest.
/// </summary>
public sealed class Release1TheEnvelopeQuest : Quest
{
    public const string DisplayTitle = "The Envelope";

    private readonly QuestEntry[] _entries;

    public Release1TheEnvelopeQuest()
    {
    ...
    public void Project(Release1DesiredQuest quest)
    {
        ...
        if (quest.Entries.Count != _entries.Length)
            throw new InvalidOperationException(
                $"Release 1 The Envelope quest desired entry count ({quest.Entries.Count}) does not match the quest's native entry count ({_entries.Length}).");
