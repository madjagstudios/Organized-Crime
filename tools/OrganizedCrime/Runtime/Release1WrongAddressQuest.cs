...
/// <summary>
/// The Wrong Address quest rendered through S1API. The installed S1API build does carry a naming
...
/// <see cref="DisplayTitle"/> doubles as both the player-facing title and the S1API lookup key for
/// this quest. Structurally identical to <see cref="Release1SmallCourtesyQuest"/>: two entries
/// created at construction, projected and read back the same way.
/// </summary>
public sealed class Release1WrongAddressQuest : Quest
{
    public const string DisplayTitle = "Wrong Address";

    private readonly QuestEntry[] _entries;

    public Release1WrongAddressQuest()
    {
    ...
    public void Project(Release1DesiredQuest quest)
    {
        ...
        if (quest.Entries.Count != _entries.Length)
            throw new InvalidOperationException(
                $"Release 1 Wrong Address quest desired entry count ({quest.Entries.Count}) does not match the quest's native entry count ({_entries.Length}).");
