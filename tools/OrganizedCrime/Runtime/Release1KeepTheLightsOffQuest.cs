...
/// <summary>
/// The Keep the Lights Off quest rendered through S1API. Structurally identical to
/// <see cref="Release1ShortNoticeQuest"/>, except this quest's one entry never carries a marker, at
/// any stage: nothing is ever selected at any dead drop or HQ closet for this mission (Decision 13),
/// so <see cref="Release1DesiredEntry.Marker"/> is always null in every plan Task 6 builds for it, and
/// <see cref="ApplyEntry"/> below (unchanged from Short Notice) simply projects that null straight
/// through to <see cref="QuestEntry.POIPosition"/> as <see cref="Vector3.zero"/>. Located through
/// <c>QuestManager.GetQuestByName</c>, which matches the quest's (get-only, abstract)
/// <see cref="Title"/>; <see cref="DisplayTitle"/> doubles as both the player-facing title and the
/// S1API lookup key for this quest.
/// </summary>
public sealed class Release1KeepTheLightsOffQuest : Quest
{
    public const string DisplayTitle = "Keep the Lights Off";

    private readonly QuestEntry[] _entries;

    public Release1KeepTheLightsOffQuest()
    {
    ...
    public void Project(Release1DesiredQuest quest)
    {
        ...
        if (quest.Entries.Count != _entries.Length)
            throw new InvalidOperationException(
                $"Release 1 Keep the Lights Off quest desired entry count ({quest.Entries.Count}) does not match the quest's native entry count ({_entries.Length}).");
