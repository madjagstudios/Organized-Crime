...
/// <summary>
/// The Room With No Name quest rendered through S1API. Structurally identical to
/// <see cref="Release1WrongAddressQuest"/> and <see cref="Release1SmallCourtesyQuest"/>, except this
/// quest carries three entries created at construction (collect, hold, hand off) rather than two.
...
/// </summary>
public sealed class Release1RoomWithNoNameQuest : Quest
{
    public const string DisplayTitle = "A Room With No Name";

    private readonly QuestEntry[] _entries;

    public Release1RoomWithNoNameQuest()
    {
    ...
    /// construction, three); a mismatch throws rather than silently dropping extra entries or
    /// leaving missing ones at their placeholder state, so the boundary's catch-all maps it to a
    /// logged, reported failure instead of a silent partial projection.
    /// </summary>
    public void Project(Release1DesiredQuest quest)
    {
        ...
        if (quest.Entries.Count != _entries.Length)
            throw new InvalidOperationException(
                $"Release 1 Room With No Name quest desired entry count ({quest.Entries.Count}) does not match the quest's native entry count ({_entries.Length}).");
